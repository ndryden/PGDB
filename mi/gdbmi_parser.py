...
class GDBMIParser:
    ...
    def parse_output(self, src):
        ...
        for line in lines:
            ...
            if line == self._term:
                ...
                parts = re.match(r"([0-9]*)(" + "|".join(["\\" + item for item in self._all_record_symbols]) + ")(.*)", line)
                if not parts:
                    record = GDBMIRecord()
    ...
    def parse_result_record(self, token, src):
        ...
        parts = re.match(r"(" + "|".join(self._result_class.keys()) + ")(.*)", src)
        if not parts:
    ...
    def parse_async_output(self, src):
        ...
        match = re.match(r"([a-zA-Z0-9_\-]*)(\,.*)?", src)
        if not match:
            raise ValueError(src)
